...
- **Tutorial:** el marco de cada pantalla lleva `tour` (`tourDe(ruta, brand,
  url)`) y `pagina()` lo dibuja como un cuadro fijo sobre la pantalla real. Los
  pasos salen del estado real (hay key, hay marca, hay hechos, hay pieza); el que
  se muestra sale de `?tour=N` cuando viene —asi "Siguiente" adelanta uno que
  todavia no hiciste— y si no, del primero que falte. **No ejecuta nada**: cada
  paso apunta a la pantalla que lo resuelve (`/ajustes`, `/marcas`, `/crear`). No
  agregar formularios ahi: tenerlos hacia que la misma cosa se pudiera hacer en
  dos lugares y confundia lo que se muestra con lo que se crea. `/empezar` quedo
  como redireccion al paso pendiente, por los links viejos. El `?` de la barra
  postea `tour-reiniciar`; `tour-listo` guarda `ui:tour-visto = "1"`, que gana
  sobre el calculo porque si no el tutorial volvia solo al borrar una marca de
  prueba.
